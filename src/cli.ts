...
Options:
  ...
  if (scope === "overview") {
    ...
    if (values.compact || values["compact-out"]) {
      if (values.compact) console.log(`\n${overviewStr}`);
  ...
  console.log(`JSON size: ${jsonKb.toFixed(1)} KB`);
  ...
  if (values["compact-out"] || values.compact) {
